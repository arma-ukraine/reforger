...
{spaces}}}"""
    ...
    def generate_shop_config(self, shop_config: ShopConfig) -> str:
        ...
        if not has_buyable_items:
            raise ValueError(
                f"Shop '{shop_config.name}' must have at least one buyable item (buy_items) or craft recipe"
            )
    ...
    def generate_entity_catalog_entry(self, item: Item) -> str:
        """Generate single EntityCatalog entry for an item"""
        entry_guid = self.generate_guid()
        data_guid = self.generate_guid()

        return f"""  SCR_EntityCatalogEntry "{{{entry_guid}}}" {{
   m_sEntityPrefab "{item.prefab_path}"
   m_aEntityDataList {{
    SCR_ArsenalItem "{{{data_guid}}}" {{
     m_iSupplyCost {item.base_price}
    }}
   }}
  }}"""

    def generate_entity_catalog(self, items: List[Item]) -> str:
        """Generate complete EntityCatalog configuration for all items"""
        lines = [
            "SCR_EntityCatalog {",
        ...
        for item in items:
            lines.append(self.generate_entity_catalog_entry(item))

        lines.extend([" }", "}"])
        return "\n".join(lines)

    def generate_entity_catalog_file(self, output_file: Path) -> Path:
        """Generate EntityCatalog file with all items from the Item enum"""
        # Get all items from the Item enum
        all_items = list(Item)

        catalog_content = self.generate_entity_catalog(all_items)
        ...
        print(f"Generated EntityCatalog: {output_file}")
        return output_file

    def generate_arsenal_item(self, item: Item) -> str:
        """Generate single ArsenalShops item entry"""
        item_guid = self.generate_guid()

        return f"""  SCR_ArsenalItemStandalone "{{{item_guid}}}" {{
   m_eItemType EQUIPMENT
   m_ItemResourceName "{item.prefab_path}"
  }}"""

    def generate_arsenal_config(self, shop_config: ShopConfig) -> str:
        """Generate complete ArsenalShops configuration for a shop"""
        lines = ["SCR_ArsenalItemListConfig {", " m_aArsenalItems {"]
        ...
        # Generate arsenal items
        for item in all_items:
            lines.append(self.generate_arsenal_item(item))

        lines.extend([" }", "}"])
        return "\n".join(lines)

    def generate_all_arsenal_shops(self, output_dir: Path) -> List[Path]:
        """Generate all ArsenalShops configuration files"""
        output_dir.mkdir(parents=True, exist_ok=True)
        generated_files: List[Path] = []

        for shop in Shop:
            shop_config = shop.value
            config_content = self.generate_arsenal_config(shop_config)
            file_path = output_dir / f"AUA_SHOP_{shop_config.name}.conf"

            with open(file_path, "w", encoding="utf-8") as f:
                f.write(config_content)

            generated_files.append(file_path)
            print(f"Generated ArsenalShop: {file_path}")
